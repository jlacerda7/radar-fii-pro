# --- VERSÃO DE PRODUÇÃO (CLOUD) v19 ---
# --- MOTOR API (BRAPI.DEV) - RESTAURANDO O P/VP COM 'MODULES' ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V19 - PEDINDO DADOS FUNDAMENTALISTAS) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V19 - P/VP)...")
    ...
    try:
        ...
        for i, lote in enumerate(lotes_de_fiis):
            try:
                ...
                # Pedimos os dados E o módulo de dados fundamentalistas (que contém o bookValue)
                fiis_data_response = brapi.quote.retrieve(tickers=lote, modules="summaryProfile")
                # --- FIM DA MUDANÇA ---
        ...
        # --- PARTE 4: PROCESSAMENTO (LÓGICA "BULLETPROOF" V17/V19) ---
        dados_para_db = []
        for fii in todos_os_dados:
            
            # 4.1 Verificamos se os atributos ESSENCIAIS existem
            ticker = getattr(fii, 'stock', None)
            price = getattr(fii, 'regular_market_price', None)
            book_val = getattr(fii, 'bookValue', None) # Agora esperamos que isso exista!
            
            # 4.2 Verificamos o atributo OPCIONAL (DY)
            dy_val = getattr(fii, 'dividend_yield', None)
            
            # 4.3 Se os dados essenciais (Ticker, Preço, Valor) existirem e forem válidos...
            if ticker and price and book_val and book_val > 0:
                pvp = price / book_val
                dy = dy_val * 100 if dy_val else 0 
                dados_para_db.append((ticker, pvp, dy))
            
            # Se um FII não tiver 'bookValue' (mesmo com o 'modules'), 
            # ele será ignorado, e o app NÃO VAI QUEBRAR.
        
    except Exception as e:
        st.error(f"Erro ao conectar ou processar dados da API Brapi: {e}")
        print(f"Erro V19: {e}") # Log para nós
        return False
    ...
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    # V19: Voltamos a salvar P_VP e DY_12M
    cursor.executemany("""
...
# --- PARTE 2: APP WEB (LÓGICA DO SCORE V1) ---
...
# V19: Voltamos ao cálculo do Score Pro V1 (DY x P/VP)
def calcular_score_pro(df):
    if df.empty:
        df['Score Pro'] = pd.Series(dtype='int')
        return df
    
    # Filtro de qualidade: P/VP razoável e paga dividendos
    ...
    if (df_filtrado['DY_12M'].max() - df_filtrado['DY_12M'].min()) == 0 or (df_filtrado['P_VP'].max() - df_filtrado['P_VP'].min()) == 0:
        ...
        df_filtrado['pvp_norm'] = 50 # Nota P/VP
    else:
...
st.title("🛰️ Radar FII Pro (Cloud V19 - P/VP Ativado)")
