# --- VERSÃO DE PRODUÇÃO (CLOUD) v23 ---
# --- MOTOR API (BRAPI.DEV) - CORREÇÃO FINAL (LIMPEZA DE TICKERS) ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V23 - LIMPEZA DE TICKERS) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V23)...")

    try:
        api_key = st.secrets["BRAPI_API_KEY"]
        brapi = Brapi(api_key=api_key)

        # 1. Pedimos a lista de TODOS os FIIs
        fii_list_response = brapi.quote.list(type="fund")
        # Limpeza inicial: Garantir que só pegamos tickers válidos (ex: MXRF11)
        fii_tickers_brutos = [fii.stock for fii in fii_list_response.stocks]
        # Regex para validar tickers (4 letras, número 11)
        ...
        for i, lote in enumerate(lotes_de_fiis):
            try:
                # --- GARANTIA V23: Assegura que 'lote' é uma lista de strings limpas ---
                lote_limpo = [str(t).strip() for t in lote if isinstance(t, str)]
                if not lote_limpo: continue # Pula lote vazio ou inválido
                ...
                st.warning(f"Falha ao buscar o lote {i+1} ({lote_limpo}). Erro: {e_lote}")
...
st.title("🛰️ Radar FII Pro (Cloud V23 - Final)")
