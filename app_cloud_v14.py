# --- VERSÃO DE PRODUÇÃO (CLOUD) v14 ---
# --- MOTOR API (BRAPI.DEV) - CORREÇÃO FINAL (USA QUOTE.RETRIEVE) ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V14 - CORRIGIDA) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V14)...")
    
    try:
        api_key = st.secrets["BRAPI_API_KEY"]
        brapi = Brapi(api_key=api_key)
        
        fii_list_response = brapi.quote.list(type="fund") # Correto do V13
        fii_tickers = [fii.stock for fii in fii_list_response.stocks] # Correto do V13
        ...
        # --- ESTA É A CORREÇÃO (V14) ---
        # Antes: fiis_data = brapi.get_stocks(stock=fii_tickers) <-- ERRADO
        fiis_data_response = brapi.quote.retrieve(tickers=fii_tickers) # <-- CORRETO
        
        dados_para_db = []
        
        # 4. Processamos os dados (temos que pegar de .results)
        # Antes: for fii in fiis_data: <-- ERRADO
        for fii in fiis_data_response.results: # <-- CORRETO
        # --- FIM DA CORREÇÃO ---
        ...
        print(f"Erro V14: {e}") # Log para nós
        return False
...
st.title("🛰️ Radar FII Pro (Cloud V14 - API Engine)")
