# --- VERSÃO DE PRODUÇÃO (CLOUD) v11 ---
# --- MOTOR API (BRAPI.DEV) - CORREÇÃO DO BUG "NOT CALLABLE" ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V11 - CORRIGIDA) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V11)...")
    ...
    try:
        ...
        # --- ESTA É A CORREÇÃO ---
        # Antes: response_data = brapi.available() <-- ERRADO (bug)
        response_data = brapi.available # <-- CORRETO (sem parênteses)
        # --- FIM DA CORREÇÃO ---
        ...
        for fii in fiis_data:
            # P/VP e DY vêm de locais diferentes na resposta da API
...
st.title("🛰️ Radar FII Pro (Cloud V11 - API Engine)")
