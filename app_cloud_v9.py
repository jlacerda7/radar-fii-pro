# --- VERSÃO DE PRODUÇÃO (CLOUD) v9 ---
# --- PIVOT: FUNDAMENTUS (ROBÔ DISFARÇADO) ---
# --- USA REQUESTS + REFERER ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V9 - ROBÔ DISFARÇADO) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Iniciando busca por TODOS os FIIs no Fundamentus (Robô V9 Disfarçado)...")
    
    try:
        # 1. DISFARCE: Fingimos ser um navegador vindo do Google
        headers = {
        ...
        # 2. Baixa a página
        response = requests.get(URL_FUNDAMENTUS_FIIS, headers=headers, timeout=15)
        
        # Log de depuração (aparecerá nos logs do Streamlit)
        print(f"FUNDAMENTUS V9: Status Code = {response.status_code}")
        
        if response.status_code != 200:
            st.error(f"Falha ao acessar o Fundamentus. Status: {response.status_code}")
            return False
        ...
        if not tabela_fiis:
            st.error("Não foi possível encontrar a tabela de dados no Fundamentus. O site pode ter bloqueado o acesso.")
            # Log de depuração
            print(f"HTML Recebido (DEBUG): {response.text[:500]}")
            return False

        # 4. Converte a tabela HTML em um DataFrame Pandas
        ...
        print(f"Erro V9: {e}") # Log de depuração
        return False
...
# --- O RESTANTE DO CÓDIGO (PARTE 2) É IDÊNTICO AO V8 ---
...
st.title("🛰️ Radar FII Pro (Cloud V9 - Disfarçado)")
...
# --- CORREÇÃO DO BUG V9 ---
if st.sidebar.button("Forçar Atualização Agora (Rápido)"): 
    atualizar_dados_fiis()
    df_base = carregar_dados_do_db()
    st.rerun() # <<< COMANDO CORRIGIDO (ESTÁVEL)
