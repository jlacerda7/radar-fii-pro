# --- VERSÃO DE PRODUÇÃO (CLOUD) v12 ---
# --- MOTOR API (BRAPI.DEV) - CORREÇÃO DO BUG ".STOCKS" ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V12 - CORRIGIDA) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V12)...")
    
    try:
        api_key = st.secrets["BRAPI_API_KEY"]
        brapi = Brapi(api_key=api_key)
        
        response_data = brapi.available # Correto do V11
        
        # --- ESTA É A CORREÇÃO (V12) ---
        # Antes: response_data.tickers <-- ERRADO (bug)
        fiis_list = [ticker for ticker in response_data.stocks if "11" in ticker] # <-- CORRETO (.stocks)
        # --- FIM DA CORREÇÃO ---
...
st.title("🛰️ Radar FII Pro (Cloud V12 - API Engine)")
...
colunas_para_exibir = {'Ticker': 'Ticker', 'Score Pro': 'Score Pro 🔥', 'P_VP': 'P_VP', 'DY_12M': 'DY (12M) %'}
