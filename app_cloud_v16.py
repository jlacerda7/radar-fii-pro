# --- VERSÃO DE PRODUÇÃO (CLOUD) v16 ---
# --- MOTOR API (BRAPI.DEV) - CORREÇÃO FINAL (bookValue) ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V16 - CORRIGIDA) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V16)...")
    ...
    try:
        ...
        for fii in todos_os_dados:
            ...
            # Antes: pvp = fii.regular_market_price / fii.book_value if fii.book_value ... <-- ERRADO
            pvp = fii.regular_market_price / fii.bookValue if fii.bookValue and fii.bookValue > 0 else 0 # <-- CORRETO (bookValue)
            # --- FIM DA CORREÇÃO ---
        ...
        print(f"Erro V16: {e}") # Log para nós
        return False
...
st.title("🛰️ Radar FII Pro (Cloud V16 - API Pro)")
