# --- VERSÃO DE PRODUÇÃO (CLOUD) v17 ---
# --- MOTOR API (BRAPI.DEV) - CÓDIGO "BULLETPROOF" (À PROVA DE FALHAS) ---
...
# --- FUNÇÃO ATUALIZAR_DADOS (V17 - LÓGICA DE LOTES) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V17)...")
    ...
    try:
        ...
        # --- PARTE 4: PROCESSAMENTO (LÓGICA "À PROVA DE FALHAS" V17) ---
        dados_para_db = []
        for fii in todos_os_dados:
            
            # 4.1 Verificamos se os atributos ESSENCIAIS existem
            # Usamos 'getattr' para verificar 'com segurança'
            ticker = getattr(fii, 'stock', None)
            price = getattr(fii, 'regular_market_price', None)
            book_val = getattr(fii, 'bookValue', None) # Sim, o nome estava certo no V16
            ...
            if ticker and price and book_val and book_val > 0:
                ...
                # Processa o DY (se existir, usa; senão, é 0)
                dy = dy_val * 100 if dy_val else 0 
                
                dados_para_db.append((ticker, pvp, dy))
            
            # Se um FII não tiver 'bookValue' ou 'regular_market_price', 
            # ele será simplesmente ignorado, e o loop continuará.
            # O APP NÃO VAI MAIS QUEBRAR.
        
    except Exception as e:
        st.error(f"Erro ao conectar ou processar dados da API Brapi: {e}")
        print(f"Erro V17: {e}") # Log para nós
        return False
...
st.title("🛰️ Radar FII Pro (Cloud V17 - Bulletproof)")
