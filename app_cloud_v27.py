# --- VERSÃO DE PRODUÇÃO (CLOUD) v27 ---
# --- MOTOR API (BRAPI.DEV) - EXTRAÇÃO DE DADOS DA STRING COM REGEX ---
...
import math
import time
import re # <<< Ferramenta de extração de texto
...
# --- FUNÇÃO ATUALIZAR_DADOS (V27 - EXTRAÇÃO COM REGEX) ---
def atualizar_dados_fiis():
    status_placeholder = st.empty()
    status_placeholder.info("Conectando à API Brapi para buscar TODOS os FIIs (V27)...")
    ...
    try:
        ...
        regex_fii_valid = re.compile(r"^[A-Z]{4}11$") # Renomeado para clareza
        ...
        for i, lote in enumerate(lotes_de_fiis):
            ...
            try:
                ...
                print(f"[AVISO V27] Lote {i+1} erro HTTP {status_code}: {lote_limpo}. Erro: {http_err}")
            except Exception as e_lote:
                erros_lote += 1
                print(f"[ERRO V27] Falha genérica lote {i+1}: {lote_limpo}. Erro: {e_lote}")
        ...
        # --- PARTE 4: PROCESSAMENTO (LÓGICA "EXTRATOR" V27) ---
        dados_para_db = []
        if not todos_os_dados: # Verifica se a lista está vazia
             st.error("Nenhum dado foi coletado com sucesso após processar todos os lotes.")
             print("[ERRO V27] Lista 'todos_os_dados' está vazia.")
             return False

        for fii in todos_os_dados:
            ticker = getattr(fii, 'stock', None)
            # Pegamos a STRING do módulo
            stats_string = getattr(fii, 'default_key_statistics', None)

            # Só processa se tiver ticker e a string do módulo
            if ticker and stats_string:
                pvp = 0.0
                dy = 0.0

                # Tenta extrair P/VP da string
                # Regex: procura "price_to_book=", captura números e ponto (ex: 0.97219425)
                match_pvp = re.search(r"price_to_book=([\d\.]+)", str(stats_string))
                if match_pvp:
                    try:
                        pvp = float(match_pvp.group(1))
                    except ValueError:
                        pvp = 0.0 # Define 0 se a conversão falhar

                # Tenta extrair DY da string
                # Regex: procura "dividend_yield=", captura números e ponto OU a palavra "None"
                match_dy = re.search(r"dividend_yield=([\d\.]+|None)", str(stats_string))
                if match_dy:
                    dy_str = match_dy.group(1)
                    if dy_str != "None":
                        try:
                            # A API retorna decimal, multiplicamos por 100
                            dy = float(dy_str) * 100
                        except ValueError:
                            dy = 0.0 # Define 0 se a conversão falhar
                    # Se dy_str for "None", dy continua 0.0

                # Adiciona ao DB apenas se tivermos um P/VP válido e DY positivo
                if pvp > 0 and dy > 0:
                    dados_para_db.append((ticker, pvp, dy))
                # Mesmo se dy for 0 (como no caso do MXRF11 no debug), se o pvp for válido,
                # poderíamos adicionar, mas o Score Pro daria 0. Mantemos o filtro dy > 0 por enquanto.

            # Se não encontrar a string do módulo ou o ticker, ignora o FII.

    except Exception as e:
        st.error(f"Erro CRÍTICO durante a execução da coleta: {e}")
        print(f"Erro CRÍTICO V27: {e}")
        return False

    status_placeholder.empty()

    if not dados_para_db:
        st.error("Dados foram coletados, mas nenhum FII continha P/VP e DY válidos na string do módulo 'defaultKeyStatistics'.")
        print("[ERRO V27] Lista 'dados_para_db' está vazia após o processamento com Regex.")
        return False
...
st.title("🛰️ Radar FII Pro (Cloud V27 - Final)")
